...
type SourceOutageSettings = { source_outage_state?: unknown }
...
export async function getTennisRecordOperationalStatus(service: SupabaseClient) {
  ...
  const [settings, lastRun, lastSuccessfulRun, recentCompletedRuns, pending, conflicts, ratingPending, identities, campaigns] = await Promise.all([
    ...
    service.from('tennisrecord_sync_runs').select('*').neq('trigger_kind', 'ratings').order('started_at', { ascending: false }).limit(1).maybeSingle(),
    service.from('tennisrecord_sync_runs').select('completed_at').neq('trigger_kind', 'ratings').eq('status', 'completed').not('completed_at', 'is', null).order('started_at', { ascending: false }).limit(1).maybeSingle(),
  ...
  const weeklyForecast = tennisRecordCheckpointForecastWithPace(
    ...
    weeklyPace,
    independentRefresh ? 'bootstrap' : 'weekly',
  )
...
export async function enqueueTennisRecordUrls(service: SupabaseClient, urls: string[], campaignId?: string | null) {
  // Keep PostgREST URL filters below practical proxy/header size limits.
  if (urls.length > 50) {
    let queued = 0
    for (let offset = 0; offset < urls.length; offset += 50) queued += await enqueueTennisRecordUrls(service, urls.slice(offset, offset + 50), campaignId)
    return queued
  }
...
async function markCurrentSeasonUrls(service: SupabaseClient, urls: string[]) {
  ...
  for (let offset = 0; offset < urls.length; offset += 50) {
    const saved = await service.from('tennisrecord_crawl_queue')
      .update({ refresh_season: season, refresh_due_at: new Date().toISOString() })
...
export async function prepareCurrentSeasonRefresh(service: SupabaseClient, runId: string, settings: Settings) {
  ...
  if (seedDue) {
    ...
    if (campaign.data) {
      ...
      if (!seedComplete) {
        // Save progress only after every URL in this page is durably queued.
        const savedCursor = await service.from('tennisrecord_collector_settings').update({ current_refresh_player_cursor: page.data![page.data!.length - 1].id, current_refresh_seed_cycle_at: new Date().toISOString() }).eq('id', true)
        if (savedCursor.error) throw new Error(savedCursor.error.message)
      }
    }
  }
...
function sourceOutageFromSettings(settings: unknown) {
  return readSourceOutageState((settings as SourceOutageSettings | null)?.source_outage_state)
}

async function sourceCooldownSummary(service: SupabaseClient, settings: unknown): Promise<TennisRecordRunSummary | null> {
  const state = sourceOutageFromSettings(settings)
  if (!sourceOutageIsCooling(state)) return null
  // A process crash must not leave ratings locked out for the full cooldown.
  // This only reclaims already-stale runs; it does not start source work.
  await reclaimStaleTennisRecordRuns(service)
  emitImporterTelemetry({ event: 'tennisrecord_source_cooldown', kind: 'skipped', retry_at: state.cooldownUntil })
  return { ...emptySummary('skipped'), reason: 'source_cooldown', retryAt: state.cooldownUntil! }
}

class SourceOutagePersistenceError extends Error {}

async function saveSourceOutage(service: SupabaseClient, state: SourceOutageState) {
  try {
    const saved = await service.from('tennisrecord_collector_settings').update({ source_outage_state: state }).eq('id', true).select('id').single()
    if (saved.error || !saved.data) throw new Error(saved.error?.message || 'Could not save source cooldown.')
  } catch (error) {
    throw new SourceOutagePersistenceError(error instanceof Error ? error.message : 'Could not save source cooldown.')
  }
}
...
export async function runTennisRecordSync(service: SupabaseClient, input: SyncInput): Promise<TennisRecordRunSummary> {
  ...
  try {
    // Settings may have changed while waiting for the shared collector/rating
    ...
    const replay = input.currentSeason
      ? { sourceMatchKeys: [], sourcePlayerKeys: [], baselineChanged: false, pagesProcessed: 0, playersDiscovered: 0, teamsDiscovered: 0, matchesStaged: 0, parserFailures: 0 }
      : await reparseCapturedTennisRecordMatchPages(service, runId, input.campaignId, campaignSlug)
    ...
    for (let index = 0; index < requestedLimit; index += 1) {
      ...
      if (!hasTennisRecordFetchBudget(sourceDeadlineAt, settings.min_request_interval_ms)) break
      const claimed = await service.from('tennisrecord_crawl_queue').update({ status: 'running', attempted_at: new Date().toISOString(), last_run_id: runId }).eq('id', job.id).eq('status', 'pending').select('id').maybeSingle()
      ...
      try {
        ...
        if (page.blockReason) {
          ...
          const blocked = await service.from('tennisrecord_crawl_queue').update({ status: 'blocked', failure_reason: page.blockReason, completed_at: new Date().toISOString() }).eq('id', job.id)
        ...
        const eventReviewReason = parsed.reviewReason || await parsedPageEventReview(service, parsed)
        if (eventReviewReason) {
          summary.parserFailures += 1
          const review = await service.from('tennisrecord_crawl_queue').update({
            status: 'review', failure_reason: eventReviewReason,
        ...
        if (sourceFailure) {
          ...
          if (sourceOutageIsCooling(outage)) {
            ...
            emitImporterTelemetry({ event: 'tennisrecord_source_cooldown', kind: 'opened', run_id: runId, retry_at: outage.cooldownUntil, level: outage.level })
            break
          }
        }
    ...
    // Do not strand a claimed row behind a terminal failed run. If releasing
    // it also fails, keep the run reclaimable by the existing stale-run path.
    const released = await service.from('tennisrecord_crawl_queue').update({ status: 'pending' }).eq('last_run_id', runId).eq('status', 'running')
    await service.from('tennisrecord_sync_runs').update({ ...(released.error ? {} : { status: 'failed', completed_at: new Date().toISOString() }), error_message: error instanceof Error ? error.message : 'Unknown sync failure' }).eq('id', runId)
    throw error
  }
}
...
export async function runAutomaticTennisRecordSync(service: SupabaseClient) {
  const [settingsResult, recentRunResult] = await Promise.all([
    ...
    service.from('tennisrecord_sync_runs').select('trigger_kind,started_at,completed_at,blocked_requests,parser_failures,source_failures,transient_retries').neq('trigger_kind', 'ratings').not('completed_at', 'is', null).order('completed_at', { ascending: false }).limit(1).maybeSingle(),
  ])
...
export async function runScheduledTennisRecordRatingBatch(service: SupabaseClient, now = new Date()): Promise<TennisRecordRatingBatchSummary> {
  ...
  // Atomic lock shared with both collector lanes, not a check-then-run race.
  ...
  try {
    ...
    if (pendingMatchCount) {
      const { data: processed, error: processedError } = await service
        .from('tennisrecord_canonical_matches')
        .update({ rating_processed_at: new Date().toISOString() })
        .not('canonical_match_id', 'is', null)
        .is('rating_processed_at', null)
    ...
    await recordTennisRecordRatingRefreshCompletion(service, new Date(), ratingWatermark)
    const finished = await service.from('tennisrecord_sync_runs').update({ status: 'completed', completed_at: new Date().toISOString() }).eq('id', locked.data.id)
    if (finished.error) throw new Error(finished.error.message)
...
async function recordTennisRecordRatingRefreshCompletion(service: SupabaseClient, completedAt = new Date(), requestedThrough = completedAt.toISOString()) {
  const { error } = await service
    .from('tennisrecord_collector_settings')
    .update({
  ...
  const cleared = await service.from('tennisrecord_collector_settings')
    .update({ rating_recalculation_requested_at: null, rating_recalculation_reason: null })
    .eq('id', true).lte('rating_recalculation_requested_at', requestedThrough)
  ...
  for (const page of pages || []) {
    ...
    if (eventReviewReason) {
      ...
      if (!parsed.reviewReason) {
        const held = await service.from('tennisrecord_crawl_queue').update({ status: 'review', failure_reason: eventReviewReason, last_error_at: new Date().toISOString() }).eq('source_url', sourceUrl)
...
async function parsedPageEventReview(service: SupabaseClient, parsed: ReturnType<typeof parseTennisRecordMatchPage>) {
  const courts = parsed.matches.map(match => ({ fingerprint: canonicalTennisRecordFingerprint(match), sourceUrl: match.sourceUrl }))
  const reviews = await tennisRecordEventReviews(service, courts)
  if (reviews.size) return reviews.values().next().value as string
  // Existing aliases need the same protection BEFORE staging/freshness writes,
  // not only the final reconciliation check.
  for (const court of courts) {
    const existing = await service.from('tennisrecord_canonical_matches').select('canonical_match_id').eq('fingerprint', court.fingerprint).maybeSingle()
    if (existing.error) throw new Error(existing.error.message)
    if (existing.data?.canonical_match_id) {
      const reason = await associatedSourceEventReview(service, court, existing.data.canonical_match_id)
...
async function associatedSourceEventReview(service: SupabaseClient, court: { fingerprint: string; sourceUrl: string }, canonicalMatchId: string | null | undefined) {
  const associated: string[] = []
  if (canonicalMatchId) {
    for (let offset = 0; ; offset += 200) {
      const aliases = await service.from('tennisrecord_canonical_matches').select('fingerprint').eq('canonical_match_id', canonicalMatchId).order('fingerprint').range(offset, offset + 199)
      if (aliases.error || !Array.isArray(aliases.data)) throw new Error(`Could not verify associated source events: ${aliases.error?.message || 'missing evidence response'}`)
      associated.push(...aliases.data.map(row => row.fingerprint as string))
      if (aliases.data.length < 200) break
    }
    if (!associated.length) throw new Error('Canonical source association changed during event verification; retry from fresh evidence.')
  }
...
export async function reconcileTennisRecordMatches(service: SupabaseClient, sourceMatchKeys: string[], shouldRecalculateRatings = true) {
  ...
  for (const item of staged || []) {
    ...
    // Recheck after staging, before identity resolution, observation selection,
    // canonical writes or rating requests. Existing associations may be legacy
    // aliases, so inspect every fingerprint attached to that played record.
    const eventReviewReason = await associatedSourceEventReview(service, { fingerprint: item.fingerprint, sourceUrl: item.source_url }, existingCanonical.data?.canonical_match_id)
    if (eventReviewReason) {
      const held = await service.from('tennisrecord_staged_matches').update({ parse_status: 'quarantined', parse_failure_reason: eventReviewReason }).eq('id', item.id)
      if (held.error) throw new Error(held.error.message)
      const pageHeld = await service.from('tennisrecord_crawl_queue').update({ status: 'review', failure_reason: eventReviewReason, last_error_at: new Date().toISOString() }).eq('source_url', item.source_url)
      if (pageHeld.error) throw new Error(pageHeld.error.message)
      conflicts += 1
      continue
    }
    ...
    if (identities) {
      const lookup = await findExistingProductionMatch(service, { ...item, source_event_verified: true, known_canonical_match_id: existingCanonical.data?.canonical_match_id }, identities)
