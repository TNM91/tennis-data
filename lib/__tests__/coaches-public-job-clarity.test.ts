...
describe('public coaches job clarity', () => {
  ...
  it('gives coaches and players a fast public quick path', () => {
    ...
    expect(source).toContain('Choose the next coaching action and keep the player moving.')
    ...
    expect(source).toContain('Assign player work')
    expect(source).toContain('Review progress')
    expect(source).toContain('Share the next resource')
    expect(source).toContain('Follow up with a player')
  ...
  it('tracks coach jobs from public CTAs', () => {
    ...
    expect(source).not.toContain("job: 'find_coaching_support'")
