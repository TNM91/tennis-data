...
const coachAlertSource = readSource('app/components/level-up-coach-alert.tsx')
const siteShellSource = readSource('app/components/site-shell.tsx')
...
describe('Level Up weekly plan flow', () => {
  ...
  it('alerts the player before they open My Quest and clears the alert on arrival', () => {
    expect(coachRouteSource).toContain(".from('internal_notifications').insert")
    expect(coachRouteSource).toContain("notification_type: 'message'")
    expect(coachRouteSource).toContain('notificationQueued: !notificationError')
    expect(siteShellSource).toContain('<LevelUpCoachAlert />')
    expect(coachAlertSource).toContain('Open My Quest')
    expect(coachAlertSource).toContain('listInternalNotifications(userId, { unreadOnly: true')
    expect(coachAlertSource).toContain('coachAlerts.map((alert) => markInternalNotificationRead(alert.id, userId)')
    expect(coachAlertSource).toContain("pathname === '/level-up/my-quest'")
    expect(myQuestSource).toContain('id="level-up-coach-update"')
  })
})
