...
describe('Team Room live availability card', () => {
  it('uses the private availability request as the projected-lineup response source', () => {
    ...
    expect(roomApi).toContain("card.title === 'Final lineup ready to send'")
  ...
  it('posts one final-result update and edits that update for corrections', () => {
    ...
    expect(announcementServer).toContain('resultExternalMatchId: externalMatchId')
    expect(announcementServer).toContain('matchCompletedAt')
    expect(roomApi).toContain('resultExternalMatchId')
    expect(captainPage).toContain("cta: 'Review result'")
    expect(captainPage).toContain('captainResultReviewHref')
