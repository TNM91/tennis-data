...
describe('Tactical Studio player access', () => {
  ...
  it('connects Tactical Studio to a Player ID starter path before the unlock prompt', () => {
    ...
    expect(gateSource).toContain('const tacticsImproveHref = buildTacticsImproveHref(searchParams)')
    ...
    expect(gateSource).toContain("const gateIntentLabel = hasGateLevelUpCard ? 'My Lab proof board requested' : 'Improve starter board requested'")
    expect(gateSource).toContain('const gateProofReturnCue = hasGateLevelUpCard')
    expect(gateSource).toContain('const tacticsPlayerIdStarterRead = [')
    expect(gateSource).toContain("value: gateCardTitle")
    expect(gateSource).toContain("label: 'My Lab handoff'")
    expect(gateSource).toContain('value: gateProofReturnCue')
    expect(gateSource).toContain("href={getPlanUnlockHref('player_plus', tacticsImproveHref)}>See Player")
    expect(gateSource).toContain("href={getPlanUnlockHref('full_court', tacticsImproveHref)}>See Full-Court")
    expect(gateSource).toContain("ctaHref={getPlanUnlockHref('player_plus', tacticsImproveHref)}")
    expect(gateSource).toContain("secondaryHref={getPlanUnlockHref('full_court', tacticsImproveHref)}")
    expect(gateSource).toContain('secondaryLabel="Unlock Full-Court"')
    expect(gateSource).toContain('function buildTacticsImproveHref')
    expect(gateSource).toContain("const intentKeys = ['identity', 'identityLabel', 'card', 'cardTitle'] as const")
    expect(gateSource).toContain('return hasCustomIntent ? `/tactics?${nextParams.toString()}` : TACTICS_IMPROVE_HREF')
    ...
    expect(gateSource).toContain('{gateCardTitle} opens after Player unlock.')
    expect(gateSource).toContain('{gateIdentityLabel}: read the Player ID cue, adjust the court, then save the proof back to My Lab.')
    ...
    expect(gateSource).toContain('Return ${gateCardTitle} proof to My Lab after the board is saved.')
    ...
    expect(gateSource).toContain("href={getPlanUnlockHref('player_plus', tacticsImproveHref)}")
