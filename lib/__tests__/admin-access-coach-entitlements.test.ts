...
describe('admin access coach entitlement controls', () => {
  it('keeps Coach and Full-Court available in admin entitlement management', () => {
    ...
    expect(source).toContain('manage {PLAYER_TIER.name}, Coach')
    ...
    expect(source).toContain("type AccessOffer = 'permanent_full' | 'trial_14' | 'trial_30' | 'complimentary_month'")
    expect(source).toContain('Permanent all access')
    expect(source).toContain('One complimentary month')
    expect(source).toContain('Quick offer')
    expect(source).toContain('Stripe billing is unchanged')
    expect(source).toContain('profile.linked_player_name')
  })
})
