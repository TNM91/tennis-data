...
describe('Level Up custom quest persistence', () => {
  ...
  it('links saved quests into drill completion and refreshable XP progress', () => {
    ...
    expect(clientSource).toContain('QUEST_BUILDER_DRAFT_KEY')
    expect(clientSource).toContain('tiq-level-up-quest-builder-draft-v1')
    expect(clientSource).toContain('globalThis.localStorage?.setItem')
    expect(clientSource).toContain('globalThis.localStorage?.removeItem')
    expect(clientSource).toContain('parseQuestBuilderDraft')
    expect(clientSource).toContain('Draft saved on this device.')
    expect(clientSource).toContain('Reset draft')
    ...
    expect(clientSource).toContain('Coach assignment bridge')
    expect(clientSource).toContain('Assign through Coach Hub')
    expect(clientSource).toContain('Team challenge mode')
    expect(clientSource).toContain('Aggregate completion only')
    expect(clientSource).toContain('buildQuestHandoffHref')
    expect(clientSource).toContain("params.set(target === 'coach' ? 'levelUpPack' : 'levelUpChallenge', pack.id)")
