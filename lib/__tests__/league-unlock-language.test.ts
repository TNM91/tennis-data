...
describe('League unlock language', () => {
  ...
  it('uses League Office wording in user-facing locked result prompts', () => {
    ...
    expect(leagueWorkspaceSource).toContain('quickActions={access.canUseLeagueTools ? leagueHomeQuickActions : LEAGUE_HOME_LOCKED_ACTIONS}')
    expect(leagueWorkspaceSource).toContain('{access.canUseLeagueTools ? (')
