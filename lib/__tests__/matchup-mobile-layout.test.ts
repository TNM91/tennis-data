...
describe('matchup mobile layout guards', () => {
  it('collapses dense setup and comparison rows on narrow screens', () => {
    ...
    expect(matchupSource).toContain('const dynamicControlsCard: CSSProperties')
    expect(matchupSource).toContain("padding: isMobile ? '8px' : controlsCard.padding")
    expect(matchupSource).toContain("borderRadius: isMobile ? '16px' : controlsCard.borderRadius")
    expect(matchupSource).toContain('const dynamicEmptyState: CSSProperties')
    expect(matchupSource).toContain('const matchupUpgradePrompt = (')
    expect(matchupSource).toContain('ariaLabel="Matchup upgrade plan"')
    expect(matchupSource).toContain('const dynamicToolHeaderStyle: CSSProperties')
    expect(matchupSource).toContain("paddingBottom: isMobile ? '5px' : toolHeaderStyle.paddingBottom")
    expect(matchupSource).toContain('const dynamicToolHeaderTitleStyle: CSSProperties')
    expect(matchupSource).toContain("fontSize: isMobile ? '1.12rem' : toolHeaderTitleStyle.fontSize")
    expect(matchupSource).toContain('const dynamicSelectionProgressCard: CSSProperties')
    expect(matchupSource).toContain("padding: isMobile ? '6px 8px' : selectionProgressCard.padding")
    expect(matchupSource).toContain("borderRadius: isMobile ? '999px' : selectionProgressCard.borderRadius")
    expect(matchupSource).toContain('const dynamicSelectionProgressLabel: CSSProperties')
    expect(matchupSource).toContain("fontSize: isMobile ? '10px' : selectionProgressLabel.fontSize")
    expect(matchupSource).toContain('const dynamicSelectionProgressValue: CSSProperties')
    expect(matchupSource).toContain("fontSize: isMobile ? '13px' : selectionProgressValue.fontSize")
    expect(matchupSource).toContain('const dynamicToggleGroup: CSSProperties')
    expect(matchupSource).toContain("padding: isMobile ? 3 : toggleGroup.padding")
    expect(matchupSource).toContain('const dynamicToggleButton: CSSProperties')
    expect(matchupSource).toContain("padding: isMobile ? '7px 8px' : toggleButton.padding")
    expect(matchupSource).toContain("padding: isMobile ? '8px 9px' : matchupDetailsSummaryStyle.padding")
    expect(matchupSource).toContain("{isMobile ? 'Open' : cue}")
    ...
    expect(matchupSource).toContain('const mobileQuickReadItems = projection')
    expect(matchupSource).toContain("label: 'Edge'")
    expect(matchupSource).toContain("label: 'Gap'")
    expect(matchupSource).toContain("label: 'Watch'")
    expect(matchupSource).toContain('aria-label="Matchup quick read"')
    expect(matchupSource).toContain('style={mobileQuickReadGridStyle(isSmallMobile)}')
    expect(styleBlock('mobileQuickReadItemStyle')).toContain('minWidth: 0')
    expect(styleBlock('mobileQuickReadItemStyle')).toContain("overflowWrap: 'anywhere'")
  ...
  it('keeps Matchup support grids and labels from expanding past the shell', () => {
    ...
    expect(functionBlock('SelectField')).toContain('const { isMobile } = useViewportBreakpoints()')
    expect(functionBlock('SelectField')).toContain('const dynamicInputLabel: CSSProperties')
    expect(functionBlock('SelectField')).toContain("marginBottom: isMobile ? 5 : inputLabel.marginBottom")
    expect(functionBlock('SelectField')).toContain('const dynamicSelectStyle: CSSProperties')
    expect(functionBlock('SelectField')).toContain("height: isMobile ? '46px' : selectStyle.height")
    expect(functionBlock('SelectField')).toContain('<label htmlFor={selectId} style={dynamicInputLabel}>{label}</label>')
    expect(functionBlock('SelectField')).toContain('...dynamicSelectStyle')
    ...
    expect(styleBlock('headToHeadHistoryStackStyle')).toContain("maxWidth: '100%'")
    expect(styleBlock('headToHeadHistoryStackStyle')).toContain("overflowWrap: 'anywhere'")
