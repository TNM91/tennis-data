...
describe('profile sync review workflow', () => {
  ...
  it('excludes reviewed sync repair failures from the open review queue', () => {
    ...
    expect(productEventsPage).toContain('const [search, setSearch] = useState')
    expect(productEventsPage).toContain("setQueryParam(params, 'search', search.trim())")
    expect(productEventsPage).toContain('function eventMatchesSearch(event: ProductUsageEventRow, rawSearch: string)')
    expect(productEventsPage).toContain('id="product-event-search"')
