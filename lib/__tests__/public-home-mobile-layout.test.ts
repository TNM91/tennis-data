...
describe('Public home mobile layout guards', () => {
  ...
  it('keeps the persistent mobile portal as a docked two-level icon palette', () => {
    ...
    expect(portalToolbarSource).toContain("aria-label={mobilePortalLane ? `${mobilePortalLane.label} actions` : customizingPortalLanes ? 'Personalize main TenAceIQ menu' : 'Main TenAceIQ menu'}")
    ...
    expect(portalToolbarSource).toContain('onSelect={customizingPortalLanes ? handlePortalLaneCustomization : handleMobilePortalLaneSelect}')
    ...
    expect(portalToolbarSource).toContain('if (lane) router.push(lane.route)')
