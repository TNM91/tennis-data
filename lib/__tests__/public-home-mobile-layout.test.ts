...
describe('Public home mobile layout guards', () => {
  ...
  it('keeps the persistent mobile portal as a docked two-level icon palette', () => {
    ...
    expect(portalToolbarSource).toContain("data-mobile-portal-palette={mobilePortalLane ? 'actions' : customizingPortalShortcuts ? 'edit' : 'shortcuts'}")
    expect(portalToolbarSource).toContain("aria-label={mobilePortalLane ? `${mobilePortalLane.label} actions` : customizingPortalShortcuts ? 'Personalize TenAceIQ shortcuts' : 'Pinned TenAceIQ shortcuts'}")
    ...
    expect(portalToolbarSource).toContain('function MobilePortalShortcutTile')
    expect(portalToolbarSource).toContain('data-portal-shortcut={shortcut.id}')
