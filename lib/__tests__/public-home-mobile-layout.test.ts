...
describe('Public home mobile layout guards', () => {
  ...
  it('keeps the active public command-center homepage mobile-safe', () => {
    ...
    expect(styleBlock(commandCenterSource, 'heroTitleStyle')).toContain("fontSize: 'clamp(2rem, 7cqw, 3.25rem)'")
    ...
    expect(commandCenterSource).toContain('HomeIntelligenceSnapshot')
    ...
    expect(commandCenterSource).toContain('Show reads')
    ...
    expect(commandCenterSource).toContain('Show paths')
    ...
    expect(styleBlock(commandCenterSource, 'modeCardStyle')).toContain('minHeight: 138')
    expect(styleBlock(commandCenterSource, 'modeActionStyle')).toContain('minHeight: 44')
    expect(commandCenterSource).toContain('<summary style={compactSummaryStyle}>What opens</summary>')
    expect(commandCenterSource).toContain('<summary style={compactSummaryStyle}>Use when</summary>')
    ...
    expect(styleBlock(commandCenterSource, 'closingBandStyle')).toContain("flexWrap: 'wrap'")
    expect(styleBlock(commandCenterSource, 'closingBandStyle')).toContain('minWidth: 0')
    expect(styleBlock(commandCenterSource, 'closingCopyStyle')).toContain("flex: '1 1 360px'")
    expect(styleBlock(commandCenterSource, 'closingTitleStyle')).toContain("overflowWrap: 'anywhere'")
    expect(styleBlock(commandCenterSource, 'closingActionRowStyle')).toContain("flexWrap: 'wrap'")
    expect(styleBlock(commandCenterSource, 'closingActionRowStyle')).toContain('minWidth: 0')
    ...
    expect(commandCenterSource).toContain('export function GuestTierPreview()')
    expect(commandCenterSource).toContain('Start free. Add only what helps.')
    expect(styleBlock(commandCenterSource, 'guestTierGridStyle')).toContain(
  ...
  it('uses one top portal shell instead of switching pages into a side rail', () => {
    expect(siteShellSource).toContain('<PortalToolBar suppressed={compactSiteMenuOpen} />')
    expect(siteShellSource).toContain('<SiteHeader')
    ...
    expect(siteShellSource).not.toContain('portalRailLayoutStyle')
    expect(siteShellSource).not.toContain('portalRailScrollStyle')
  })
  ...
  it('keeps the persistent mobile portal as a docked two-level icon palette', () => {
    ...
    expect(portalToolbarSource).toContain("const collapseMobilePortal = layout === 'top'")
    ...
    expect(portalToolbarSource).toContain("data-mobile-portal-palette={mobilePortalLane ? 'actions' : customizingPortalShortcuts ? 'edit' : showAllPortalLanes ? 'all-tools' : 'shortcuts'}")
    expect(portalToolbarSource).toContain("aria-label={mobilePortalLane ? `${mobilePortalLane.label} actions` : customizingPortalShortcuts ? 'Personalize TenAceIQ shortcuts' : showAllPortalLanes ? 'All TenAceIQ tools' : 'Pinned TenAceIQ shortcuts'}")
    ...
    expect(portalToolbarSource).toContain("aria-label={customizing ? `${pinned ? 'Unpin' : 'Pin'} ${label}` : `${label}: ${lane.cue}`}")
    ...
    expect(portalToolbarSource).toContain('...(mobilePortalLane ? mobilePortalActionPaletteStyle : mobilePortalPaletteStyle)')
    expect(portalToolbarSource).toContain("'repeat(8, minmax(0, 1fr))'")
    expect(portalToolbarSource).toContain('data-mobile-portal-scrollbar="true"')
    expect(portalToolbarSource).toContain('mobilePortalScrollbarStyle')
    expect(portalToolbarSource).toContain('mobilePortalScrollbarThumbStyle')
    expect(portalToolbarSource).toContain('handleMobilePortalPaletteScroll')
    expect(portalToolbarSource).toContain('mobilePortalScrollProgress')
    ...
    expect(portalToolbarSource).toContain('data-mobile-portal-all="open"')
    ...
    expect(portalToolbarSource).toContain('expanded={false}')
    expect(portalToolbarSource).toContain('aria-controls={customizing ? undefined : controlsId}')
    expect(portalToolbarSource).toContain('aria-expanded={customizing ? undefined : expanded}')
    ...
    expect(styleBlock(portalToolbarSource, 'mobilePortalPaletteStyle')).toContain("gridTemplateColumns: 'repeat(4, minmax(0, 1fr))'")
    expect(portalToolbarSource).toContain("? mobilePortalLane")
    expect(portalToolbarSource).toContain(": 'repeat(4, minmax(0, 1fr))'")
    expect(styleBlock(portalToolbarSource, 'mobilePortalPaletteStyle')).toContain("width: '100%'")
    expect(styleBlock(portalToolbarSource, 'mobilePortalPaletteStyle')).toContain("boxSizing: 'border-box'")
    expect(styleBlock(portalToolbarSource, 'mobilePortalPaletteStyle')).toContain("overflow: 'hidden'")
    expect(styleBlock(portalToolbarSource, 'mobilePortalPaletteStyle')).not.toContain('scrollbarColor')
    expect(styleBlock(portalToolbarSource, 'mobilePortalPaletteStyle')).not.toContain("gridAutoColumns: 'minmax(70px, 1fr)'")
    expect(styleBlock(portalToolbarSource, 'mobilePortalPaletteStyle')).not.toContain('scrollSnapType')
    expect(globalsSource).toContain('[data-mobile-portal-palette]::-webkit-scrollbar')
    expect(globalsSource).toContain('scrollbar-width: none')
    expect(globalsSource).toContain('scrollbar-gutter: auto')
    expect(globalsSource).toContain('-ms-overflow-style: none')
    expect(globalsSource).toContain('display: none')
    expect(globalsSource).toContain('height: 0')
    expect(globalsSource).not.toContain('[data-mobile-portal-palette]::-webkit-scrollbar-track')
    expect(globalsSource).not.toContain('[data-mobile-portal-palette]::-webkit-scrollbar-thumb')
    ...
    expect(styleBlock(portalToolbarSource, 'mobilePortalTileStyle')).toContain('minHeight: 64')
    expect(portalToolbarSource).toContain("const portalLaneOrder: PortalLaneId[] = ['find', 'you', 'compete', 'team', 'coach', 'league', 'club']")
    expect(portalToolbarSource).toContain('if (lane) {')
    ...
    expect(styleBlock(portalToolbarSource, 'mobilePortalTileIconStyle')).toContain('width: 32')
  ...
  it('keeps public hub routes mapped to the right mobile portal lane', () => {
    expect(portalToolbarSource).toContain("label: 'Teams'")
    expect(portalToolbarSource).toContain("cue: 'Your teams and match week'")
    expect(portalToolbarSource).toContain("if (laneId === 'team') return 'Teams'")
    expect(portalToolbarSource).toContain("paths: ['/coach', '/coaches', '/player-development', '/tactics']")
    expect(portalToolbarSource).toContain("paths: ['/captain', '/manage', '/compete/teams', '/team-room']")
