...
describe('Manage hub actions', () => {
  it('puts the fastest Manage path directly under the hero', () => {
    ...
    expect(source).toContain('What do you need to run?')
    expect(source).toContain('Open the tool for the job in front of you.')
    expect(source).not.toContain('What are you trying to organize right now?')
    expect(source).not.toContain('Choose the closest path, then TenAceIQ keeps the next step focused on less admin and more tennis.')
    expect(source).toContain('manageQuickPaths')
    expect(source).toContain('Set lineup and message')
    expect(source).toContain('Run league season')
    expect(source).toContain('Run event desk')
    expect(source).toContain('Review score or roster')
    expect(source).toContain('data-manage-path-job={path.job}')
  })

  it('frames each management path around the practical user question', () => {
    expect(source).toContain('Need more detail before choosing?')
    expect(source).not.toContain('Start with the need causing the chaos.')
    expect(source).not.toContain('Start with the job that is causing the chaos.')
    expect(source).toContain('Show paths')
    expect(source).toContain('Open the right tool.')
    expect(source).toContain('Use these when you want more detail before choosing.')
  ...
  it('connects Manage operations to a Player ID starter path', () => {
    ...
    expect(source).toContain('className="manageDetailsSection" style={managePlayerIdTrailStyle}')
    expect(source).toContain('Player ID to operations')
    expect(source).toContain('Use player proof before decisions get messy.')
    expect(source).toContain('Show context')
    expect(source).toContain('Manage Player ID starter read')
    expect(source).toContain('managePlayerIdStarterRead.map((item)')
    expect(source).toContain('Start Level Up')
    expect(source).toContain('Read Player ID')
    expect(source).toContain('Open Team Hub')
    expect(source.indexOf('Manage Player ID starter path')).toBeLessThan(source.indexOf('Manage paths'))
  })
  ...
  it('keeps the question treatment compact and mobile-safe', () => {
    ...
    expect(source).toContain("gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 145px), 1fr))'")
    ...
    expect(source).toContain('const managePlayerIdTrailStyle')
    expect(source).toContain('const manageDetailsSummaryStyle')
    expect(source).toContain('const detailsSectionStyle')
    expect(source).toContain('className="manageDetailsSection" style={detailsSectionStyle}')
    expect(globalsSource).toContain('.manageDetailsSection:not([open]) > :not(summary)')
    expect(source).toContain("display: 'block'")
    expect(source).toContain('manage-actions-detail-title')
    expect(source).toContain('const managePlayerIdGridStyle')
    expect(source).toContain("gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 176px), 1fr))'")
    expect(source).toContain('const managePlayerIdActionRowStyle')
    expect(source).toContain("flexWrap: 'wrap'")
    expect(source).toContain('const managePlayerIdActionStyle')
