...
describe('admin growth funnel', () => {
  it('keeps visitor traffic separate from signed-in product and billing conversion signals', () => {
    ...
    expect(pageSource).toContain('Needs Pilot form')
    expect(routeSource).toContain('buildCaptainPilotClaimFollowUps')
    expect(routeSource).toContain('loadCaptainPilotSignupIdentities')
    ...
    expect(routeSource).toContain('buildFollowGrowthCohort')
    expect(routeSource).toContain("from('upgrade_requests')")
    expect(pageSource).toContain('Player requests after click')
    expect(pageSource).toContain('First useful action')
