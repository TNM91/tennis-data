...
describe('player development question strip', () => {
  ...
  it('links each player question to a practical action path', () => {
    ...
    expect(systemSource).toContain("href={`/player-development/${identity.slug}/level-up${firstCard ? `?card=${firstCard.id}` : ''}`}")
    expect(systemSource).toContain("href={`/level-up/${identity.slug}`}")
