...
describe('Data Assist trust engine', () => {
  ...
  it('gives upload visitors a plain source refresh path before the file picker', () => {
    ...
    expect(source).toContain('For team setup, start with Team Summary. TiQ reviews every source before records change.')
    expect(source).toContain('For team setup, start with Team Summary.')
    expect(source).toContain('Team setup: start with Team Summary.')
    expect(source).toContain('Use Player Roster later only if you manage that team and want its phone or email contacts.')
