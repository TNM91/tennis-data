...
describe('Data Assist trust engine', () => {
  ...
  it('gives upload visitors a plain source refresh path before the file picker', () => {
    ...
    expect(source).toContain('Choose a file')
    expect(source).toContain('What are you uploading?')
    ...
    expect(source).toContain("const teamSetupRequested = !contactImportRequested && requestedImportType === 'team_summary' && intentContext.toLowerCase().includes('team')")
    expect(source).toContain('{intent && !teamSetupRequested ? <DataAssistIntentPanel')
    expect(source).toContain("<StepBadge step={1} label={teamSetupRequested ? 'Add a team' : contactImportRequested ? 'Team contacts' : 'Data Assist'} />")
    expect(source).toContain('Add your team.')
    expect(source).toContain('Choose the Team Summary export from TennisLink.')
    ...
    expect(source).toContain('Choose your Team Summary.')
    expect(source).toContain('Adds team, roster, ratings and flight')
    expect(source).toContain('Review the team, then connect it to My Teams.')
