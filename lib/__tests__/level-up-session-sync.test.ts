...
describe('Level Up session sync', () => {
  ...
  it('persists Level Up work for players and exposes shared logs to coaches', () => {
    ...
    expect(workbenchSource).toContain('cardId: session.sessionJson.cardId')
    expect(workbenchSource).toContain('cardId: savedSourceCard?.id')
    expect(portalSource).toContain('cardId: card.id')
  ...
  it('surfaces Level Up proof on coach assignment review cards', () => {
    ...
    expect(coachPageSource).toContain('buildCoachProofStarterRead(levelUpProof, proofReviewStandard, proofReviewDraft)')
    ...
    expect(coachPageSource).toContain('if (!session.starterRead && !standard && !draft) return null')
    expect(coachPageSource).toContain('`${session.drillTitle}: ${session.focusTitle}`')
    expect(coachPageSource).toContain("standard?.items.find((item) => item.label === 'Player counted')?.value")
    expect(coachPageSource).toContain("`${session.rating}/5 proof score")
    expect(coachPageSource).toContain("standard?.items.find((item) => item.label === 'Coach checks')?.value")
    expect(coachPageSource).toContain("standard?.items.find((item) => item.label === 'Next rep')?.value")
    expect(coachPageSource).toContain("'Choose repeat, simplify, or add pressure from this proof.'")
    ...
    expect(coachPageSource).toContain('Proof review order for ${assignment.title}')
    expect(coachPageSource).toContain('1. Read the proof')
    expect(coachPageSource).toContain('2. Choose the next move')
    expect(coachPageSource).toContain('3. Message or assign')
    expect(coachPageSource).toContain('proofReviewOrderStyle')
    expect(coachPageSource).toContain('proofReviewOrderGridStyle')
    ...
    expect(coachPageSource).toContain('proofReviewPrimaryActionStyle')
    expect(coachPageSource).toContain('proofReviewSecondaryButtonStyle')
    expect(coachPageSource).toContain('proofReviewSecondaryLinkStyle')
