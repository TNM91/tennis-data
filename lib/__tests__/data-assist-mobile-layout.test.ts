...
describe('Data Assist mobile layout guards', () => {
  ...
  it('keeps parsed review rows and screenshot cards wrapped on narrow screens', () => {
    ...
    expect(styleBlock('parsedLineStyle')).toContain("'repeat(auto-fit, minmax(min(100%, 8rem), 1fr))'")
    expect(styleBlock('bulkResultRowStyle')).toContain("'repeat(auto-fit, minmax(min(100%, 8rem), 1fr))'")
    expect(styleBlock('screenshotCardStyle')).toContain("gridTemplateColumns: isMobile ? 'minmax(0, 1fr)'")
