...
describe('matchup prep path', () => {
  ...
  it('keeps the detailed Match Prep private to Player access', () => {
    expect(source).toContain('const playerMatchPrep = useMemo(() => {')
    expect(source).toContain('Your Match Prep')
    expect(source).toContain('Player-only read')
    expect(source).toContain('Current context')
    expect(source).toContain('Recent evidence')
    expect(source).toContain('Past meetings')
    expect(source).toContain('Court plan')
    expect(source).toContain('{access.canUseAdvancedPlayerInsights && playerMatchPrep ? (')
    expect(source).toContain('playerMatchPrepGridStyle(isSmallMobile)')
    expect(styleBlock('playerMatchPrepCardStyle')).toContain('minWidth: 0')
    expect(styleBlock('playerMatchPrepCardStyle')).toContain("overflow: 'hidden'")
    expect(styleBlock('playerMatchPrepItemStyle')).toContain("overflowWrap: 'anywhere'")
  })
})
