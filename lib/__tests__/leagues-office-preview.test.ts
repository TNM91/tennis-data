...
const source = readFileSync(join(process.cwd(), 'app/leagues/page.tsx'), 'utf8').replace(/\r\n/g, '\n')
...
describe('Leagues Office preview', () => {
  ...
  it('keeps the public league directory compact on phones', () => {
    ...
    expect(source).toContain('mobileSummaryStatusStyle')
    expect(source).toContain('role="status"')
    expect(source).not.toContain('mobileSummaryPillRowStyle')
    ...
    expect(source).toContain('leagueSeasonPulseStyle')
    expect(source).toContain('leaguePulseGridStyle')
    expect(source).toContain('LeaguePulseMetric')
