...
describe('profile link API resilience', () => {
  it('keeps profile linking alive when optional profile columns are not available yet', () => {
    ...
    expect(source).toContain('const existingProfile = await loadProfileLink')
    expect(source).toContain('linked_team_name: cleanString(existingProfile?.linked_team_name) || null')
