...
describe('League hub action alignment', () => {
  it('keeps the Compete hub focused on match preparation instead of League Office operations', () => {
    ...
    expect(compete).toContain('Prep your next matchup')
    expect(compete).toContain('Scout player')
    expect(compete).toContain('Build lineup')
    expect(compete).toContain('Check results')
    expect(compete).toContain('Read a team')
    expect(compete).toContain('Understand the flight')
    expect(compete).toContain('Need help with match prep?')
