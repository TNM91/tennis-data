...
describe('Explore responsive surfaces', () => {
  ...
  it('keeps team detail pages Data Assist aware and light-mode readable', () => {
    ...
    expect(teamDetailSource).toContain('const matchTypeSplitRowStyle: CSSProperties')
    expect(teamDetailSource).toContain('const recentFormRowStyle: CSSProperties')
    expect(teamDetailSource).toContain("gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 150px), 1fr))'")
    expect(teamDetailSource).toContain('const playerRatingStackStyle: CSSProperties')
    expect(teamDetailSource).toContain('const opponentBreakdownHeaderStyle: CSSProperties')
    expect(teamDetailSource).toContain('function seasonFilterButtonStyle(active: boolean): CSSProperties')
    expect(teamDetailSource).toContain("WebkitOverflowScrolling: 'touch'")
