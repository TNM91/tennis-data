...
describe('Explore responsive surfaces', () => {
  ...
  it('keeps league detail pages Data Assist aware and light-mode readable', () => {
    ...
    expect(leagueDetailSource).toContain('const matchQualityRowStyle: CSSProperties')
    expect(leagueDetailSource).toContain('function standingsViewToggleButtonStyle(active: boolean): CSSProperties')
    expect(leagueDetailSource).toContain('const teamWinPercentRowStyle: CSSProperties')
    expect(leagueDetailSource).toContain("gridTemplateColumns: isSmallMobile ? '1fr' : 'repeat(2, minmax(0, 1fr))'")
    expect(leagueDetailSource).toContain("gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 132px), 1fr))'")
