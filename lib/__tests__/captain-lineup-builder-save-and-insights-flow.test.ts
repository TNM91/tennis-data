...
describe('Captain lineup builder save and insights flow', () => {
  ...
  it('explains captain-recorded Yes replies and makes the final send, image, and scorecard handoff explicit', () => {
    ...
    expect(source).toContain("const finalLineupDeliveryLabel = openingFinalDelivery ? 'Sending lineup…' : 'Send lineup to Team Chat'")
    ...
    expect(source).toContain('Send this confirmed lineup to Team Chat; the next screen has the image and one-page scorecard.')
