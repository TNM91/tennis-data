...
describe('League result book paths', () => {
  it('gives Team Results a clear first-action path', () => {
    expect(teamSource).not.toContain("import { PRODUCT_MOTTO } from '@/lib/product-story'")
    ...
    expect(teamSource).toContain('Create the match, add lines, check the book, or upload scorecards.')
  ...
  it('gives Player Results a clear first-action path', () => {
    expect(playerSource).not.toContain("import { PRODUCT_MOTTO } from '@/lib/product-story'")
    ...
    expect(playerSource).toContain('Choose players, save scores, check standings, or upload scorecards.')
  ...
  it('keeps result path cards tappable on mobile', () => {
    ...
    expect(teamSource).toContain("gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 240px), 1fr))'")
    expect(teamSource).toContain('minHeight: 112')
    expect(playerSource).toContain("gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 240px), 1fr))'")
    expect(playerSource).toContain('minHeight: 112')
  })
})
