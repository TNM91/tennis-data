...
describe('suite panel navigation deduplication', () => {
  ...
  it('keeps the persistent portal from replacing page-level headings', () => {
    ...
    expect(portal).toContain('const useCompactPortalControls = publicVisitor || !isMobile')
    expect(portal).toContain('const useDenseDesktopPortalRail = authenticated && !isMobile')
    expect(portal).toContain('compact={useCompactPortalControls}')
    expect(portal).toContain('dense={useDenseDesktopPortalRail}')
    expect(portal).toContain('desktopPortalMemberLaneGridStyle')
    expect(portal).toContain('signedInPortalTitleStyle')
    ...
    expect(myLab).toContain("const myLabTitle = isProfileConfirmed ? welcomeLine : 'Connect your player to start.'")
