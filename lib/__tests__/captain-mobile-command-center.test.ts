...
describe('Captain mobile command center', () => {
  ...
  it('puts the timely captain jobs on one compact mobile surface', () => {
    ...
    expect(page).toContain("const scopedMember = isMember(role) && role !== 'admin'")
    expect(page).toContain('if (!scopedMember) {')
    expect(page).toContain('for (const row of matchRows)')
    ...
    expect(page).toContain('const captainMobileTeamPulse = [')
    expect(page).toContain('aria-label="Captain team pulse"')
    expect(page).toContain("label: 'Next up'")
    expect(page).toContain("label: 'Availability'")
    expect(page).toContain("label: 'Lineup confidence'")
    expect(page).toContain('weekAtGlance.opponentLabel')
    expect(page).toContain('captainCourtConfidencePercent')
    expect(styles).toContain('.pulseRail')
    expect(styles).toContain('grid-template-columns: repeat(2, minmax(0, 1fr));')
    expect(styles).toContain('.pulse:last-child:nth-child(odd)')
    ...
    expect(page).toContain('const captainHomePrimaryAction = captainLateArrivalAction || captainPostArrivalAction || captainArrivalFollowUpAction ||')
    expect(page).toContain('getCaptainPostArrivalAction({')
    expect(page).toContain("state: 'Change ready'")
    expect(page).toContain("label: 'Prepare the scorecard'")
    expect(page).toContain("label: 'Send team recap'")
    expect(page).toContain("label: 'Upload scorecard / close week'")
    ...
    expect(page).toContain("captainPostArrivalStep === 'send_team_recap'")
    expect(page).toContain("captainPostArrivalStep === 'close_week'")
