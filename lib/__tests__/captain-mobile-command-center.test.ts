...
describe('Captain mobile command center', () => {
  ...
  it('puts the timely captain jobs on one compact mobile surface', () => {
    ...
    expect(page).toContain("captainArrivalFollowUp ? 'Follow up arrivals' : 'Team chat'")
    expect(page).toContain('teamRoomSummary.unreadCount')
    expect(chatAction).toContain('`${captainArrivalFollowUp.count} still waiting`')
    expect(chatAction).toContain('captainArrivalFollowUpHref || teamRoomHref')
    expect(chatAction).toContain('Boolean(captainArrivalFollowUp) || teamRoomSummary.unreadCount > 0')
    ...
    expect(page).toContain('const captainHomePrimaryAction = captainArrivalFollowUpAction || captainCourtPrimaryAction ||')
    expect(page).toContain('const captainHomePrimaryStatus = captainArrivalFollowUpAction')
