...
describe('public tournament match-day actions', () => {
  it('keeps participant actions compact and status-driven', () => {
    ...
    expect(source.indexOf('id="enter-tournament"')).toBeLessThan(
      source.indexOf('aria-label="Event-day command board"'),
    )
  ...
  it('connects tournament detail match day back into Player ID and Level Up prep', () => {
    ...
    expect(source).toContain('detailDisclosureStyle')
    expect(source).toContain('detailDisclosureSummaryStyle')
    ...
    expect(source).toContain('Show trust signals')
  })
  ...
  it('keeps the tournament detail Player ID read compact on phones', () => {
    ...
    expect(source).toContain("gridTemplateColumns: isMobile ? 'repeat(3, minmax(0, 1fr))' : playerRailStyle.gridTemplateColumns")
    expect(source).toContain('compactEventDayCopyStyle')
    expect(source).toContain('compactPlayerRailCardStyle')
    expect(source).toContain('compactDetailDisclosureSummaryStyle')
    ...
    expect(styleBlock('compactEventDayCopyStyle')).toContain('padding: 10')
    expect(styleBlock('compactPlayerRailCardStyle')).toContain('minHeight: 50')
    expect(styleBlock('compactDetailDisclosureSummaryStyle')).toContain("padding: '9px 10px'")
