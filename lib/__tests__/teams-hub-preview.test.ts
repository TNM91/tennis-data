...
describe('Teams Hub preview', () => {
  ...
  it('keeps repeated team-card trust details on request', () => {
    ...
    expect(source).toContain('Team form')
    expect(source).toContain("{row.wins}W - {row.losses}L")
    expect(source).toContain('Last five')
    expect(source).toContain('teamSnapshotStyle')
    expect(source).toContain("from('tennisrecord_public_team_context')")
    expect(source).toContain("source: 'tennisrecord'")
  ...
  it('gives the leading public team a compact pulse and keeps captain value as an upgrade path', () => {
    ...
    expect(source).toContain('Team context ready')
