...
describe('calendar scheduling UX', () => {
  ...
  it('keeps Messages calendar quick-add wired to the player calendar endpoint', () => {
    ...
    expect(messagesSource).toContain('messageCueItems.find((item) => !item.saved) ?? messageCueItems[0]')
    expect(messagesSource).toContain('cueCount')
    expect(messagesSource).toContain('unsavedCueCount')
    expect(messagesSource).toContain('cues in thread')
    expect(messagesSource).toContain('still unsaved')
    ...
    expect(messagesSource).toContain('Save next cue')
