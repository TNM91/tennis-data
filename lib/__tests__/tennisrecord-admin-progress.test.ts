...
describe('TennisRecord Admin import progress', () => {
  ...
  it('shows separate live historical and weekly delivery-style trackers', () => {
    ...
    expect(adminPage).toContain('label="Priority" title="Missouri-first nationwide refresh"')
    expect(adminPage).toContain('Due Missouri pages get priority.')
    expect(adminPage).toContain('Nationwide weekly queued')
    expect(adminPage).toContain('Nationwide oldest queued')
    ...
    expect(service).toContain("const TENNISRECORD_STATUS_COUNT = { count: 'planned' as const, head: true }")
  })
