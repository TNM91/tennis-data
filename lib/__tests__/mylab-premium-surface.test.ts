...
describe('My Lab premium surface', () => {
  ...
  it('pulls Level Up proof into My Lab return state with honest local sync copy', () => {
    ...
    expect(source).toContain("const MY_LAB_TACTICS_BOARD_HREF = '/tactics?source=improve&template=crosscourt&role=player'")
    expect(source).toContain("label: 'Board'")
    expect(source).toContain('Map the court shape')
    expect(source).toContain('Turn the same Level Up cue into a starter tactic board before the next court block.')
