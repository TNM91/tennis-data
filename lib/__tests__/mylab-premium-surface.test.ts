...
describe('My Lab premium surface', () => {
  ...
  it('pulls merged Level Up proof into My Lab with account-aware sync copy', () => {
    ...
    expect(source).toContain("questHref: `/level-up/${record.identitySlug}?questCard=${encodeURIComponent(record.cardId)}#quest-builder`")
    ...
    expect(source).toContain('const pathPanelTacticsHref = buildMyLabTacticsBoardHref(')
    expect(source).toContain('tacticsBoardHref={pathPanelTacticsHref}')
    expect(source).toContain('href={tacticsBoardHref}')
    expect(source).not.toContain('MY_LAB_TACTICS_BOARD_HREF')
    expect(source).toContain("label: 'Board'")
    ...
    expect(source).toContain('title: `Map ${todayHabitTitle}`')
    ...
    expect(source).toContain('Build board')
    expect(source).toContain('This proof is synced and ready for coach-linked follow-up.')
    expect(source).toContain('Account proof can support the next lesson ask when coach sharing is on.')
    ...
    expect(source).toContain('No Level Up proof yet')
    ...
    expect(source).toContain('The newest account proof follows you across signed-in devices.')
    expect(source).toContain('Signed-in account proof takes priority when available.')
    expect(source).toContain('My Lab refresh check')
    expect(source).toContain('Refresh check')
