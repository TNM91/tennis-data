...
describe('public loading state polish', () => {
  ...
  it('keeps rankings, teams, and leagues from leading with raw zero-state counts', () => {
    ...
    expect(rankingsSource).toContain('Search every location')
    ...
    expect(playersSource).toContain("value={loading ? 'Refreshing' : String(players.length)}")
    expect(playersSource).toContain("value={loading ? 'Starter' : shouldShowPlayerResults")
    expect(playersSource).not.toContain("value={loading ? '-'")
    expect(teamsSource).toContain('Search for a team or browse by league.')
    expect(teamsSource).toContain('loadingInlineStyle')
    expect(teamsSource).toContain('Search, filter, or open Captain tools while the reviewed team list refreshes.')
    expect(teamsSource).toContain("value={loading ? 'Refreshing' : String(totals.teams)}")
    expect(teamsSource).toContain("value={loading ? 'Roster sync' : String(totals.players)}")
    expect(teamsSource).not.toContain("value={loading ? '-'")
    ...
    expect(leaguesSource).toContain("value={loading ? 'Refreshing' : String(summary.totalLeagues)}")
    expect(leaguesSource).toContain("value={loading ? 'Starter' : String(summary.totalFlights)}")
    expect(leaguesSource).not.toContain("value={loading ? '-'")
