...
describe('Player Development public cleanup', () => {
  it('positions the public page as a workbook and coach-planner product story', () => {
    ...
    expect(systemSource).toContain('Standalone workbook. {playerTier.name} connected path.')
    ...
    expect(systemSource).toContain('What ${playerTier.name} adds to this workbook')
  ...
  it('keeps workbook and coach planner pages practical enough to print and use', () => {
    ...
    expect(systemSource).toContain('Start Level Up Today')
    ...
    expect(systemSource).toContain('Coach Planner')
