...
describe('compete teams readiness', () => {
  ...
  it('keeps team facts distinct from the clickable team actions', () => {
    const card = readFileSync(join(process.cwd(), 'app/compete/teams/team-home-card.tsx'), 'utf8')
    expect(source).toContain('<TeamHomeCard')
    expect(card).toContain('Next match')
    expect(card).toContain('dateTime={props.nextMatch!.date}')
    expect(card).toContain('styles.cardHeader')
    expect(card).toContain('styles.cardActions')
    expect(card).toContain('Team roster & matches')
    expect(card).toContain('aria-label={`Open ${props.name} team roster and matches`}')
    expect(card).toContain('Add matches to calendar')
    expect(card).toContain('aria-label={`Add ${props.name} matches to your calendar`}')
    expect(card).toContain('href={`${props.teamHref}#team-schedule`}')
    ...
    expect(source).toContain('lineupHref={canStartTeamLineup ? continuationHref : undefined}')
    expect(source).toContain('lineupContinuation={canStartTeamLineup && lineupContinuation')
    expect(source).toContain('availabilityHref={isCaptainTeamConnection(group.connection.roles)')
  ...
  it('keeps the empty Teams state actionable and mobile-safe', () => {
    ...
    expect(source).toContain("pendingTeamCount > 0 ? '/team-connections' : '/profile#profile-identity'")
