...
describe('admin review UI system', () => {
  ...
  it('keeps the Admin dashboard on the shared admin review shell', () => {
    ...
    expect(adminDashboardSource).toContain('title="What needs attention?"')
    expect(adminDashboardSource).toContain('href="/admin/clubs"')
    expect(adminDashboardSource).toContain('Review tennis data, account access, and club workspaces from one place.')
    expect(adminDashboardSource).not.toContain('title="Elite Admin Dashboard"')
    expect(adminDashboardSource).not.toContain('one premium')
    expect(adminDashboardSource).toContain('<DataQualityPanel />')
    expect(adminDashboardSource).toContain('<summary style={moreToolsSummaryStyle}>More admin tools')
    expect(adminDashboardSource).toContain('className="admin-tool-grid"')
    expect(adminDashboardSource).toContain("gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 224px), 1fr))'")
    expect(adminDashboardSource).toContain("href: '/admin/clubs'")
    expect(adminDashboardSource).toContain('permanently remove club workspaces')
    ...
    expect(adminDashboardSource).toContain("gridTemplateColumns: 'minmax(0, 1fr)'")
    expect(adminDashboardSource).toContain("justifySelf: 'end'")
  ...
  it('keeps Import Queue on the shared admin review shell', () => {
    ...
    expect(importQueueSource).toContain('ariaLabel="Import outcome check"')
    expect(importQueueSource).toContain('Import outcome check')
    expect(importQueueSource).toContain('Review what changed and what stayed guarded')
    expect(importQueueSource).not.toContain('Admin import outcome proof cue')
    expect(importQueueSource).not.toContain('Prove what changed and what stayed guarded')
  ...
  it('keeps Access Control on the shared admin review shell', () => {
    ...
    expect(accessSource).toContain('ariaLabel="Access repair check"')
    expect(accessSource).toContain('Access repair check')
    expect(accessSource).toContain('Check the test profile first, then update only the intended tennis access.')
    expect(accessSource).not.toContain('Admin access repair proof cue')
    expect(accessSource).not.toContain('Prove the access repair uses a test profile')
  ...
  it('keeps Manage Players on the shared admin review shell', () => {
    ...
    expect(managePlayersSource).toContain("query = query.or(`name.ilike.%${escapedSearch}%,location.ilike.%${escapedSearch}%`)")
    expect(managePlayersSource).toContain('Search uses the full player roster.')
    expect(managePlayersSource).toContain('function escapePostgrestSearch(value: string)')
