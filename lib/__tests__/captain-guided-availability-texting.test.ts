...
describe('Captain guided availability texting', () => {
  ...
  it('prepares a durable private court text before exposing a native Messages tap in the Builder', () => {
    ...
    expect(builderSource).toContain('function markPreparedCourtTextOpened(preparedText: PreparedCourtText)')
    expect(builderSource).toContain('href={preparedText.href}')
    const askAnchor = builderSource.slice(builderSource.indexOf('href={preparedText.href}'), builderSource.indexOf('</a>', builderSource.indexOf('href={preparedText.href}')))
    expect(askAnchor).toContain('onClick={() => onOpenPreparedCourtText(preparedText)}')
    expect(askAnchor).not.toContain('event.preventDefault()')
    expect(builderSource).toContain('setSmsFallback({ href: preparedText.href, playerName: preparedText.playerName })')
