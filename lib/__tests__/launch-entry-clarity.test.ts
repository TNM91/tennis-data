...
describe('launch entry clarity', () => {
  it('does not expose cached personal proof in the signed-out My Lab preview', () => {
    const guard = myLab.indexOf('if (!accessPending && !userId && !canUseAdvancedPlayerInsights)')
    const mainWorkspace = myLab.indexOf('return (\n    <section style={pageStyle}>', guard + 1)

    expect(guard).toBeGreaterThan(-1)
    expect(mainWorkspace).toBeGreaterThan(guard)
    expect(myLab.slice(guard, mainWorkspace)).toContain('completedSessions={0}')
    expect(myLab.slice(guard, mainWorkspace)).toContain('postRepReturn={null}')
    expect(myLab.slice(guard, mainWorkspace)).toContain('repCta="Create free account"')
  })
