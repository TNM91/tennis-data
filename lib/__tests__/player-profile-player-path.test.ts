...
describe('player profile player path', () => {
  ...
  it('keeps profile explanations and empty panels out of the default path', () => {
    ...
    expect(source).toContain('{hasTrackedMatches ? (')
    expect(source).toContain('aria-label="Recent scorecards"')
    expect(source).toMatch(/const dynamicRightColumn:[\s\S]*?display: 'grid'/)
    expect(source).not.toContain("display: isMobile ? 'none' : 'grid'")
