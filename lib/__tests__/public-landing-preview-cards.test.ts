...
describe('public landing preview cards', () => {
  ...
  it('keeps homepage section copy approachable instead of workspace-heavy', () => {
    ...
    expect(commandCenterSource).toContain('Start with the problem in front of you.')
    expect(commandCenterSource).toContain('Improve is for practice. Compete is for match prep. Manage is for the people and schedule work around the tennis.')
    expect(commandCenterSource).toContain('The homepage should point you to one useful next move')
    expect(commandCenterSource).toContain('Practice lane')
    expect(commandCenterSource).toContain('Match lane')
    expect(commandCenterSource).toContain('Season lane')
    expect(commandCenterSource).toContain('Use when the question is what to work on next.')
    expect(commandCenterSource).toContain('Signals')
    expect(commandCenterSource).toContain("pillar.proof.join(' / ')")
    expect(commandCenterSource).not.toContain('Lane ${index + 1}')
    expect(commandCenterSource).not.toContain('Improve, compete, and manage without the extra noise.')
    expect(commandCenterSource).not.toContain('Three lanes cover the common tennis work')
