...
describe('Captain projected lineup confirmation flow', () => {
  ...
  it('automatically anchors confirmed players while retaining a deliberate captain unlock', () => {
    ...
    expect(source).toContain("isConfirmedReleased ? 'Re-lock' : 'Mark Yes & lock'")
  ...
  it('uses known opponent court history for projections and explains when no opponent roster is available', () => {
    ...
    expect(source).toContain('function applyRecentHistoricalOpponentLineup()')
    expect(source).toContain('Prefill from last lineup')
    expect(source).toContain('Select players manually')
    expect(source).toContain('Choose opponent court setup')
