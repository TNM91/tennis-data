...
describe('Captain projected lineup confirmation flow', () => {
  ...
  it('automatically anchors confirmed players while retaining a deliberate captain unlock', () => {
    ...
    expect(source).toContain("'Undo Yes — ask again'")
    expect(source).toContain('onUndoConfirmation={resetPlayerConfirmation}')
  ...
  it('lets invited players answer future match dates without signing in', () => {
    ...
    expect(page).toContain('if (data.lockedPlayer) return data.lockedPlayer')
    expect(page).toContain('Your reply is already saved. Joining TiQ is optional.')
    expect(page).toContain('{selectedPlayer && saved ? (')
  ...
  it('creates private response links and lets captains reopen the live board', () => {
    ...
    expect(collectionRoute).toContain('player.playerName.toLowerCase()')
    ...
    expect(builderRoute).toContain("const resetConfirmation = body?.status === 'pending'")
    expect(builderRoute).toContain(".delete()")
    expect(builderRoute).toContain("status: 'pending'")
    expect(builderRoute).toContain(".expireTag(`captain-lineup:${auth.userId}:${normalizeTeamRoomKey(teamName)}`)")
  })
})
