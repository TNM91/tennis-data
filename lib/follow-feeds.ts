...
export async function isFollowing(record: FollowRecord) {
  ...
  return follows.some(
    (item) => item.entity_type === record.entity_type && item.entity_id === record.entity_id,
  )
}
...
export async function appendFeedEvent(type: FeedEventType, record: FollowRecord) {
  const title = type === 'followed' ? `Following ${record.entity_name}` : `Unfollowed ${record.entity_name}`
  const body =
    type === 'followed'
  ...
  if (insert.error) {
    const feed = readLocal<any[]>(LOCAL_FEED_KEY, [])
