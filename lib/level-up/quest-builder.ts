...
export const LEVEL_UP_HABIT_PATHS: LevelUpHabitPath[] = [
  {
    ...
    coachPackId: 'rhythm-builder',
    teamChallengeId: 'rhythm-builder',
  ...
  },
  {
    ...
    coachPackId: 'consistency-builder',
    teamChallengeId: 'consistency-builder',
  ...
  },
  {
    ...
    coachPackId: 'point-start-routine',
    teamChallengeId: 'point-start-routine',
  ...
  },
  {
    ...
    coachPackId: 'doubles-readiness',
    teamChallengeId: 'doubles-readiness',
...
export function buildLevelUpHabitPaths(identitySlug: string) {
  return LEVEL_UP_HABIT_PATHS.map((path) => {
    ...
    return {
      ...
      questHref: primaryCard ? `/level-up/${identitySlug}?questPath=${path.id}&questCard=${primaryCard.id}#quest-builder` : `/level-up/${identitySlug}#quest-builder`,
      coachHref: `/coach?levelUpPack=${encodeURIComponent(path.coachPackId)}${primaryCard ? `&card=${encodeURIComponent(primaryCard.id)}` : ''}`,
      captainHref: `/captain?levelUpChallenge=${encodeURIComponent(path.teamChallengeId)}${primaryCard ? `&card=${encodeURIComponent(primaryCard.id)}` : ''}`,
    }
  })
}
