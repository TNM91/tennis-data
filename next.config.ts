...
const nextConfig: NextConfig = {
  ...
  async headers() {
    return [
      {
        // Bearer calendar routes supply their own isolated CSP. Applying the
        // app-wide policy here overrides their nonce and no-referrer headers.
        source: '/((?!calendar/share/|api/calendar/shared/).*)',
        headers: [
          {
      ...
      ...['/calendar/share/:path*', '/api/calendar/shared/:path*'].map(source => ({
        source,
        headers: [
          { key: 'X-Content-Type-Options', value: 'nosniff' },
          { key: 'X-Frame-Options', value: 'DENY' },
