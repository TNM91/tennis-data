      ...
      {tab === 'people' ? (
        <PeoplePanel
          ...
          onInvite={async (email, roles) => {
            setWorking(true)
            try {
              const response = await request<{ invite: { inviteToken: string } }>(`/api/clubs/${workspace.club.id}/members`, { method: 'POST', body: JSON.stringify({ email, roles }) })
