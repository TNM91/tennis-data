...
export async function GET(request: Request) {
  try {
    ...
    const date = params.get('date') || '', opponent = params.get('opponent') || ''
    ...
    const [roster, invites, saved, scenarios, requests] = await Promise.all([
    ...
    if (saved.error || scenarios.error || requests.error || (saved.data?.length || 0) > 250 || (requests.data?.length || 0) > 50) throw new Error('Replies unavailable')
    ...
    const selection = scenarios.data?.length === 1 ? scenarios.data[0] : null
    return json({ summary: summarizeTeamAvailability(roster, answers, selection ? selectedLineupPlayers(selection.slots_json) : null),
      selection: selection ? 'saved' : scenarios.data?.length ? 'choose' : 'none', scenarioId: selection?.id || '',
