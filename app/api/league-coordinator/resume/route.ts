...
export async function GET(request: Request) {
  ...
  const { data, error } = await auth.service
    .from('league_coordinator_workspace_preferences')
  ...
  if (error) {
    ...
    return Response.json({ ok: false, message: 'League could not restore your last workspace.' }, { status: 500 })
  }

  const row = data as ResumeRow | null
  const resume = sanitizeLeagueCoordinatorResumeState(row?.resume_state)
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return Response.json({ ok: false, message: 'League resume data is invalid.' }, { status: 400 })
  }

  const resume = sanitizeLeagueCoordinatorResumeState(body.resume)
  if (!Object.keys(resume).length) {
    return Response.json({ ok: false, message: 'League resume data is empty.' }, { status: 400 })
  }
  ...
  if (error) {
    ...
    return Response.json({ ok: false, message: 'League could not save your last workspace.' }, { status: 500 })
  }
...
async function getResumeAuth(request: Request) {
  ...
  if (!token) {
    return { ok: false as const, response: Response.json({ ok: false, message: 'Sign in to resume League.' }, { status: 401 }) }
  }
  ...
  if (!serviceKey) {
    return { ok: false as const, response: Response.json({ ok: false, message: 'League cloud resume is not configured.' }, { status: 503 }) }
  }
...
function isMissingResumeTable(message: string | null | undefined) {
  ...
  return normalized.includes('league_coordinator_workspace_preferences')
