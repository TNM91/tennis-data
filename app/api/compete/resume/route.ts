...
export async function GET(request: Request) {
  ...
  const { data, error } = await auth.service
    .from('compete_workspace_preferences')
  ...
  if (error) {
    ...
    return Response.json({ ok: false, message: 'Compete could not restore your last work.' }, { status: 500 })
  }

  const row = data as ResumeRow | null
  const resume = sanitizeCompeteResumeState(row?.resume_state)
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return Response.json({ ok: false, message: 'Compete resume data is invalid.' }, { status: 400 })
  }

  const resume = sanitizeCompeteResumeState(body.resume)
  if (!Object.keys(resume).length) {
    return Response.json({ ok: false, message: 'Compete resume data is empty.' }, { status: 400 })
  }
  ...
  if (error) {
    ...
    return Response.json({ ok: false, message: 'Compete could not save your last work.' }, { status: 500 })
  }
...
async function getResumeAuth(request: Request) {
  ...
  if (!token) {
    return { ok: false as const, response: Response.json({ ok: false, message: 'Sign in to resume Compete.' }, { status: 401 }) }
  }
  ...
  if (!serviceKey) {
    return { ok: false as const, response: Response.json({ ok: false, message: 'Compete cloud resume is not configured.' }, { status: 503 }) }
  }
...
function isMissingResumeTable(message: string | null | undefined) {
  ...
  return normalized.includes('compete_workspace_preferences')
