...
const port = Number(process.env.TIQ_FIXTURE_PORT || 3021)
...
await build({ stdin: { contents: `import React from 'react'; import { createRoot } from 'react-dom/client'; import TeamSeasonCalendar from './app/components/team-season-calendar'; import SeasonKickoff from './app/components/season-kickoff'; import TeamAvailabilityClient from './app/team-availability/team-availability-client'; createRoot(document.getElementById('root')).render(location.pathname === '/team-availability' ? <TeamAvailabilityClient query={location.search} /> : location.pathname === '/hub-fixture' ? <SeasonKickoff scope={${JSON.stringify(scope)}} token="synthetic" onCalendar={() => {}} /> : <TeamSeasonCalendar team="Aces" matches={${JSON.stringify(matches)}} userId="test-captain" accessToken="synthetic" importHref="/unused" canStartSeason />);`, resolveDir: root, loader: 'tsx' }, bundle: true, format: 'esm', outfile: path.join(root, 'artifacts/season-kickoff-browser/fixture.js'), jsx: 'automatic', plugins: [{ name: 'fixture-link', setup(builder) {
  ...
  builder.onLoad({ filter: /.*/, namespace: 'fixture-auth' }, () => ({ contents: `export function useAuth() { const signedIn = location.pathname === '/team-availability' && !new URLSearchParams(location.search).has('signedOut'); return { session: signedIn ? { access_token: 'synthetic' } : null, userId: signedIn ? 'test-player' : null, authResolved: true } }`, loader: 'js' }))
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/phone-fixture') {
      ...
      const target = url.searchParams.get('view') === 'recipient' ? `/team-availability?${new URLSearchParams({ ...scope, match: matches[0].id }).toString().replaceAll('&', '&amp;')}` : '/hub-fixture'
      res.setHeader('Content-Type', 'text/html'); res.end(`<!doctype html><html><head><title>Synthetic phone preview</title></head><body style="margin:0;background:#ddd"><iframe title="${width} pixel season availability preview" src="${target}" style="width:${width}px;height:850px;border:0;display:block;margin:16px auto"></iframe></body></html>`); return
    }
    if (['/captain-fixture', '/hub-fixture', '/team-availability'].includes(url.pathname)) {
    ...
    if (url.pathname === '/api/team-availability') {
      const invite = invites.find(row => row.roster_key === roster[0].key)
      res.setHeader('Content-Type', 'application/json')
      if (!invite) { res.statusCode = 404; res.end(JSON.stringify({ message: 'Ask your captain to prepare the group request.' })); return }
      res.end(JSON.stringify({ responseToken: invite.response_token, focusMatchId: url.searchParams.get('match') || '' })); return
    }
    if (url.pathname.startsWith('/api/season-availability/') || url.pathname === '/api/captain/season-kickoff') {
      ...
      if (url.pathname === '/api/captain/season-kickoff') {
        if (req.method === 'POST') {
          const keys = input.action === 'group' ? roster.map(player => player.key) : input.action === 'self' ? [roster[0].key] : input.playerKeys
...
server.listen(port, '127.0.0.1', () => console.log(`Synthetic UI harness: http://localhost:${port}/captain-fixture and /hub-fixture`))
