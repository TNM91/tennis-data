...
const SCORE_REPAIR_BATCH_SIZE = 500
...
const unresolvedScoreOrientations: Array<{
  matchId: string
...
const intendedScoreChanges = eligibleMatches.flatMap((match) => (
  buildScoreOrientationRepair(match, unresolvedScoreOrientations)
))
...
if (auditAfter.scoreAudit.unparsed > auditBefore.scoreAudit.unparsed) {
  throw new Error('Post-write verification introduced additional unparsed eligible scores')
}
...
function buildScoreOrientationRepair(
  match: MatchRow,
  unresolved: Array<{
    matchId: string
    externalMatchId: string | null
