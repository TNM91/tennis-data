...
Run `npm run qa:fixture-auth-smoke` when both Day 1 `coach_primary` and `player_plus_linked` credentials are available in `.env.local` and you need to prove Coach Hub plus My Lab open without printing secrets. Use exact selectors such as `npm run qa:fixture-auth-smoke -- coach_primary`, `npm run qa:fixture-auth-smoke -- player_plus_linked`, or `npm run qa:fixture-auth-smoke -- <fixture | day3 | day4 | day5 | paid | all>` when a single role fixture is blocked.
...
| 2026-06-29 | Codex | Phone 390x844 Chromium production local-first path | player_plus_linked | player-level-up-mobile-loop | /player-development/relentless-competitor-4-0/level-up | needs-follow-up | fixture-gap | p2 | docs/qa-evidence/2026-06-29/day1/2026-06-29-day1-player-level-up-mobile-loop-active-card-phone-codex.png; docs/qa-evidence/2026-06-29/day1/2026-06-29-day1-player-level-up-mobile-loop-tiny-note-phone-codex.png; docs/qa-evidence/2026-06-29/day1/2026-06-29-day1-player-level-up-mobile-loop-saved-proof-status-phone-codex.png; docs/qa-evidence/2026-06-29/day1/2026-06-29-day1-player-level-up-mobile-loop-next-recommendation-phone-codex.png | Production mobile evidence passed the local-first Level Up loop: active card visible, 4/5 proof and tiny note saved, next recommendation and coach-update copy rendered, and local completion persisted. Authenticated Player+ My Lab return-state and cross-device sync were not proven in this run. | Confirm the `player_plus_linked` fixture with `npm run qa:fixture-auth-smoke -- player_plus_linked`, then rerun `npm run qa:live-card -- player-level-up-mobile-loop --date=2026-06-29 --tester=Codex --device=phone` while signed in. |
| 2026-06-29 | Codex | Desktop Chromium production public/gated My Lab route | player_plus_linked | player-my-lab-return-state | /mylab | needs-follow-up | fixture-gap | p2 | docs/qa-evidence/2026-06-29/day5/2026-06-29-day5-full-court-access-pass-paid-workspace-locks-desktop-codex.png | Production `/mylab` loaded a Player/My Lab entry state and correctly kept paid personalization locked without an active plan. This reused the Day 5 `/mylab` lock screenshot to avoid duplicate media, and it does not prove linked player identity, Level Up return state after refresh, or next action for a signed-in Player+ account. | Confirm `player_plus_linked` with `npm run qa:fixture-auth-smoke -- player_plus_linked` plus `linked-player-profile`, then rerun `npm run qa:live-card -- player-my-lab-return-state --date=2026-06-29 --tester=Codex --device=desktop` while signed in after a Level Up activity refresh. |
| 2026-06-29 | Codex | Desktop Chromium production signed-in Player+ fixture | player_plus_linked | player-my-lab-return-state | /mylab | pass |  |  | docs/qa-evidence/2026-06-29/day2/2026-06-29-day2-player-my-lab-return-state-linked-profile-state-desktop-codex.png; docs/qa-evidence/2026-06-29/day2/2026-06-29-day2-player-my-lab-return-state-identity-signal-desktop-codex.png; docs/qa-evidence/2026-06-29/day2/2026-06-29-day2-player-my-lab-return-state-next-action-desktop-codex.png; docs/qa-evidence/2026-06-29/day2/2026-06-29-day2-player-my-lab-return-state-post-refresh-state-desktop-codex.png; docs/qa-evidence/2026-06-29/day2/2026-06-29-day2-player-my-lab-return-state-my-lab-refresh-proof-cue-desktop-codex.png | Production signed-in Player+ pass: My Lab kept QA Linked Player as the active identity after refresh, showed linked player/profile guidance, preserved the shared Coach Hub assignment due state, and kept the coach-connected Level Up context visible without a stale upgrade lock. | Continue Day 3 captain-week-flow signed-in fixture testing. |
| 2026-06-07 | Codex | Fixture review and deterministic contract tests | coach_primary | coach-player-assigned-challenge | /coach | blocked | fixture-gap | p2 |  | `node scripts/verify-coach-player-loop.mjs` passed, but live `coach_primary`, `coach-invite-token`, linked `player_plus_linked`, `level-up-assignment`, and `level-up-completion` fixtures still need confirmation before pass evidence is honest. | Provision or confirm the coach/player linked fixtures, run `npm run qa:fixture-auth-smoke -- coach_primary` and `npm run qa:fixture-auth-smoke -- player_plus_linked`, then run `npm run qa:live-card -- coach-player-assigned-challenge --date=2026-06-07 --tester=Codex --device=phone`. |
| 2026-06-29 | Codex | Fixture auth smoke env check | coach_primary | coach-player-assigned-challenge | /coach | blocked | fixture-gap | p2 |  | `npm run qa:fixture-auth-smoke -- --env` confirmed the credential contract, and `npm run qa:fixture-auth-smoke` blocked before browser login because `TENACEIQ_QA_COACH_EMAIL`, `TENACEIQ_QA_COACH_PASSWORD`, `TENACEIQ_QA_PLAYER_EMAIL`, and `TENACEIQ_QA_PLAYER_PASSWORD` are not configured. | Add the Day 1 coach/player fixture credentials to `.env.local` or the shell, then run `npm run qa:fixture-auth-smoke -- coach_primary` and `npm run qa:fixture-auth-smoke -- player_plus_linked`; after both `/coach` and `/mylab` verify, rerun `npm run qa:live-card -- coach-player-assigned-challenge --date=2026-06-29 --tester=Codex --device=phone`. |
...
| 2026-06-29 | Codex | Desktop Chromium production public League routes | league_coordinator | league-result-to-public-context | /league-coordinator | needs-follow-up | fixture-gap | p2 | docs/qa-evidence/2026-06-29/day4/2026-06-29-day4-league-result-to-public-context-league-office-operation-proof-cue-desktop-codex.png; docs/qa-evidence/2026-06-29/day4/2026-06-29-day4-league-result-to-public-context-public-league-directory-desktop-codex.png; docs/qa-evidence/2026-06-29/day4/2026-06-29-day4-league-result-to-public-context-public-league-screen-privacy-check-desktop-codex.png | Production public League Office loaded with League access locked, `/leagues` showed league discovery and office handoff copy, and a public TIQ league detail opened without private controls visible. The selected public league had 0 matches and 0 teams, so this does not prove coordinator result entry, standings movement, or source-to-public propagation. | Provision or confirm `league_coordinator` with `npm run qa:fixture-auth-smoke -- league_coordinator` plus `league-week`, then rerun `npm run qa:live-card -- league-result-to-public-context --date=2026-06-29 --tester=Codex --device=desktop` while signed in and verify a safe result changes the intended public/member context. |
| 2026-06-29 | Codex | Desktop Chromium production protected admin routes | admin_test | admin-access-and-data-quality | /admin/access | blocked | fixture-gap | p2 | docs/qa-evidence/2026-06-29/day4/2026-06-29-day4-admin-access-and-data-quality-starting-access-desktop-codex.png; docs/qa-evidence/2026-06-29/day4/2026-06-29-day4-admin-access-and-data-quality-data-trust-guard-cue-desktop-codex.png | Production `/admin/access` and `/admin/import-queue` redirected to the same login state with the intended `next` values, and Data Assist stayed public/review-oriented. The safe `admin_test`, access repair, and data-assist upload fixtures are not configured here, so no admin access change, import review, rollback note, or affected product-surface update was attempted. | Provision or confirm `admin_test` with `npm run qa:fixture-auth-smoke -- admin_test`, `admin-access-repair`, and `data-assist-upload` in a safe fixture workspace, then rerun `npm run qa:live-card -- admin-access-and-data-quality --date=2026-06-29 --tester=Codex --device=desktop` and verify before/after access plus reviewed import impact without touching real customer data. |
...
| 2026-06-29 | 1 | 0 | 0 | 0 | 1 | My Lab public gate is healthy, but signed-in Player+ return state still needs a linked player fixture. |
...
| 2026-06-29 | 1 | 0 | 0 | 0 | 1 | Full-Court pricing and Free-state locks are healthy, but multi-role paid access still needs a Full-Court fixture. |
| 2026-06-29 | 1 | 0 | 0 | 1 | 0 | Admin route protection is healthy, but access repair and data-quality proof need safe admin fixtures. |
