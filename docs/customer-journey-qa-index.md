...
2. Run `npm run qa:session -- day1` for the active testing session.
3. Open `docs/customer-journey-weekly-runbook.md`.
4. For Day 1, run `npm run qa:day1`.
5. Run `npm run qa:fixtures` while confirming test accounts and safe data.
6. Run `npm run qa:ledger` and paste starter rows into `docs/customer-journey-test-results.md`.
7. Run `npm run qa:flows` before walking a tier end to end.
8. Run `npm run qa:focus -- <tier-or-journey>` while testing one journey at a time.
9. Run `npm run qa:handoffs` before testing linked or shared-role workflows.
10. Run focused commands as needed: `npm run qa:matrix`, `npm run qa:gaps`, `npm run qa:evidence`, or `npm run qa:triage`.
11. Run `npm run qa:results` after each testing block.
12. Run `npm run qa:launch` only after the ledger has real pass evidence.
13. Run `npm run verify:closeout:live` after the latest deploy.
