...
Run `npm run qa:flows` to print tier journey steps, handoffs, access rules, and evidence points. Run `npm run qa:trace -- <tier | journey | feature | route>` to connect tier promise, pain point, access rule, features, proving journeys, handoffs, and ledger evidence. Run `npm run qa:handoffs` to isolate cross-role proof and access checks. Run `npm run qa:matrix` to print the tier-by-feature matrix while testing. Run `npm run qa:coverage -- <tier>` to confirm each feature has a proving journey and logged pass evidence. Run `npm run qa:gaps` to focus the account, fixture, manual, and local-sync evidence pass.
...
| --- | --- | --- | --- | --- | --- |
| Free | Free/public tester | `free_viewer`, `data-assist-upload` | Signed-out or Free access can use public discovery and Data Assist entry without private workspace controls. | Public discovery proof cue, public detail screenshot, Data Assist review-first handoff cue, Data Assist upload state proof cue, review-state screenshot, and pricing handoff screenshot. | Return to Explore or Pricing and confirm the user still sees useful public context plus a clear next tier. |
| Player | Player tester | `player_plus_linked`, `linked-player-profile`, `level-up-completion` | Player access opens My Lab and Level Up; coach-linked assigned work is visible only when the relationship exists. | Mobile active-task screenshot, Level Up local sync proof cue, saved 0-5 proof screenshot, next-action screenshot, My Lab Level Up return-state panel, and My Lab refresh proof cue. | Refresh Level Up and My Lab, then confirm the Level Up return-state panel and My Lab refresh proof cue pull recent proof forward with honest local/synced copy. |
