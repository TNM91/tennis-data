...
Run `npm run qa:fixture-auth-smoke` after setting `TENACEIQ_QA_BASE_URL`, `TENACEIQ_QA_COACH_EMAIL`, `TENACEIQ_QA_COACH_PASSWORD`, `TENACEIQ_QA_PLAYER_EMAIL`, and `TENACEIQ_QA_PLAYER_PASSWORD` in `.env.local` to prove both `coach_primary` and `player_plus_linked` can authenticate without printing credentials. Use `TENACEIQ_QA_BASE_URL=https://www.tenaceiq.com` for production QA, or `http://localhost:3000` only when a local dev server is running. The same smoke also accepts selectors such as `coach_primary`, `player_plus_linked`, `captain_primary`, `league_coordinator`, `full_court_operator`, `admin_test`, `day3`, `day4`, `day5`, `paid`, and `all` when those fixture credentials are present.
