...
begin
  ...
  if v_action is null or v_action not in ('acknowledged', 'answered', 'adjusted', 'replaced') then
    raise exception 'Choose a valid coach response.' using errcode = '22023';
  end if;
  if v_action in ('answered', 'adjusted') and v_note = '' then
    raise exception 'Add your guidance before sending.' using errcode = '22023';
  end if;
  if v_action = 'answered'
    and coalesce(v_plan_json #>> '{coachResponse,playerReply,action}', '') <> 'question' then
    raise exception 'That player question has already been answered.' using errcode = '22023';
  end if;

  if v_action in ('adjusted', 'replaced') then
    select rep
      into v_target_rep
