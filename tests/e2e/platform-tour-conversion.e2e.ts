...
test('connects the platform tour to current role plans without mobile overflow', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: /Player \$1\.99\/month/ })).toBeVisible()
  ...
  await expect(recommendation).toContainText('$4.99/month')
  ...
  await expect(dialog).toContainText('$4.99/month')
  ...
  await expect(recommendation).toContainText('Club Unlimited $149/month')
